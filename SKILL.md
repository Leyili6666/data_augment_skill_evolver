...
7. 仲裁模型和 API 配置。
8. 用户最关注的评估维度；如果用户没有提供，使用任务契约、PRD 和数据类型推断。
9. 仅 `evolve` 模式：提议模型、技能构建模型和独立审计模型。
...
- `evaluation_prompt.json`：定义评估维度、rubric、任务上下文、数据契约、独立评委和可选仲裁。
  编写前可询问用户最关注的评估维度，例如格式正确性、PRD 覆盖度、多轮流程覆盖、意图/标签漂移、
  多样性、自然度、安全性或是否过度复制 seed。评估维度必须进入 prompt，并用于最终均分。
...
必须评估完整的生成数据。即使没有模型凭据，也要先运行确定性校验：
...
python3 <skill-dir>/scripts/evaluate_data.py \
  ...
  --bad-output <run-dir>/eval_bad_cases.json \
  --deterministic-only
```
